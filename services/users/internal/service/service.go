...
func New(db database.Database, notif notifier.Notifier) *Service {
	return &Service{
		db:    db,
		notif: notif,
	}
...
func (s Service) CreateUser(ctx context.Context, request *users.CreateUserRequest) (*emptypb.Empty, error) {
	...
	err = s.notif.Notify(ctx, userevents.UserChangedEvent{
		UserID: request.GetUser().GetId(),
	})
	if err != nil {
		return nil, fmt.Errorf("unable to notify create: %w", err)
	}
...
func (s Service) PatchUser(ctx context.Context, request *users.PatchUserRequest) (*emptypb.Empty, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to notify patch: %w", err)
	}
	return &emptypb.Empty{}, nil
}
...
func (s Service) DeleteUser(ctx context.Context, request *users.DeleteUserRequest) (*emptypb.Empty, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to notify delete: %w", err)
	}
	return &emptypb.Empty{}, nil
}
